...
int main(int argc, char** argv)
{
  ...
  image_transport::Subscriber sub = its.subscribe("usb_camera/image_raw",1,imageCallback);
...
*/
  cv::Mat transfMatrix;
  cv::FileStorage fs("/home/jesus/catkin_ws/homoMat4.yaml", cv::FileStorage::READ);
  if (!fs.isOpened())
  {
  ...
  while (ros::ok()) {
    ...
    if (image)
    {
      ...
      cv::Mat img = extract_lane(transformed);
